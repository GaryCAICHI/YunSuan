...
class VIAluAdder(xlen: Int = 64) extends Module {
  ...
  private val isReImSwap = opcode.isReImSwap
  private val isReNeg    = opcode.isReNeg
  private val isImNeg    = opcode.isImNeg

  private val vs2Vec = Wire(Vec(8, UInt(8.W)))
  private val vs1Vec = Wire(Vec(8, UInt(8.W)))

  vs2Vec(0) := Mux(widenVs2,
    vs2Widen(7, 0),
    Mux(isReImSwap, vs2(23, 16), vs2(7, 0))
  )

  vs2Vec(1) := Mux(widenVs2,
    Mux(sel8, Fill(8, isSigned & vs2Widen(7)), vs2Widen(15, 8)),
    Mux(isReImSwap, vs2(31, 24), vs2(15, 8))
  )

  vs2Vec(2) := Mux(widenVs2,
    Mux1H(Seq(
      sel8  -> vs2Widen(15, 8),
      sel16 -> Fill(8, isSigned & vs2Widen(15)),
      sel32 -> vs2Widen(23, 16)
    )),
    Mux(isReImSwap, vs2(7, 0), vs2(23, 16))
  )

  vs2Vec(3) := Mux(widenVs2,
    Mux(sel32, vs2Widen(31, 24), Fill(8, isSigned & vs2Widen(15))),
    Mux(isReImSwap, vs2(15, 8), vs2(31, 24))
  )

  vs2Vec(4) := Mux(widenVs2,
    Mux(sel32, Fill(8, isSigned & vs2Widen(31)), vs2Widen(23, 16)),
    Mux(isReImSwap, vs2(55, 48), vs2(39, 32))
  )

  vs2Vec(5) := Mux(widenVs2,
    Mux1H(Seq(
      sel8  -> Fill(8, isSigned & vs2Widen(23)),
      sel16 -> vs2Widen(31, 24),
      sel32 -> Fill(8, isSigned & vs2Widen(31))
    )),
    Mux(isReImSwap, vs2(63, 56), vs2(47, 40))
  )

  vs2Vec(6) := Mux(widenVs2,
    Mux(sel8, vs2Widen(31, 24), Fill(8, isSigned & vs2Widen(31))),
    Mux(isReImSwap, vs2(39, 32), vs2(55, 48))
  )

  vs2Vec(7) := Mux(widenVs2,
    Fill(8, isSigned & vs2Widen(31)),
    Mux(isReImSwap, vs2(47, 40), vs2(63, 56))
  )
  ...
  private val srcaInv = Wire(Vec(8, UInt(8.W)))
  for (i <- 0 until 4) {
    if (i % 2 == 0) {
      srcaInv(2*i)   := Mux(isReNeg, ~vs2Vec(2*i)  , vs2Vec(2*i))
      srcaInv(2*i+1) := Mux(isReNeg, ~vs2Vec(2*i+1), vs2Vec(2*i+1))
    } else {
      srcaInv(2*i)   := Mux(isImNeg, ~vs2Vec(2*i)  , vs2Vec(2*i))
      srcaInv(2*i+1) := Mux(isImNeg, ~vs2Vec(2*i+1), vs2Vec(2*i+1))
    } 
  }
  ...
  cIn(0) := Mux(isAddCarry, Mux(vm, isSub, isSub ^ mask(0)), isSub | isReNeg)
  ...
  cIn(2) := Mux1H(Seq(
    ...
    sel16 -> Mux(isAddCarry, Mux(vm, isSub, isSub ^ mask(1)), (isSub & !widenVs2 & !widen) | isImNeg),
  ))
  ...
  cIn(4) := Mux1H(Seq(
    ...
    sel16 -> Mux(isAddCarry, Mux(vm, isSub, isSub ^ mask(2)), isSub | isReNeg),
  ...
  cIn(6) := Mux1H(Seq(
    ...
    sel16 -> Mux(isAddCarry, Mux(vm, isSub, isSub ^ mask(3)), (isSub & !widenVs2 & !widen) | isImNeg),
  ))
  ...
  for (i <- 0 until 8) {
    vs2Head(i) := vs2Vec(i)(7)
    vs1Head(i) := vs1Vec(i)(7)
    vdHead(i)  := originResult(i)(7)
    if (i % 4 == 3) {
      coutSat(i) := Mux(isSigned,
      Mux(isSub,   !vs2Head(i) & vs1Head(i) &  vdHead(i) |  vs2Head(i) & !vs1Head(i) & !vdHead(i),
      Mux(isImNeg, !vs1Head(i) & vs2Head(i) &  vdHead(i) |  vs1Head(i) & !vs2Head(i) & !vdHead(i),
                    vs2Head(i) & vs1Head(i) & !vdHead(i) | !vs2Head(i) & !vs1Head(i) &  vdHead(i))),
      cout(i) ^ isSub)
    } else if (i % 4 == 1) {
      coutSat(i) := Mux(isSigned,
      Mux(isSub,   !vs2Head(i) & vs1Head(i) &  vdHead(i) |  vs2Head(i) & !vs1Head(i) & !vdHead(i),
      Mux(isReNeg, !vs1Head(i) & vs2Head(i) &  vdHead(i) |  vs1Head(i) & !vs2Head(i) & !vdHead(i),
                    vs2Head(i) & vs1Head(i) & !vdHead(i) | !vs2Head(i) & !vs1Head(i) &  vdHead(i))),
      cout(i) ^ isSub)
    } else {
      coutSat(i) := Mux(isSigned,
  ...
  for (i <- 0 until 8) {
    ...
    if (i % 4 == 2 || i % 4 == 3) {
      downOverflowSign(i) := Mux(isImNeg, selectPos(vs1Head.asUInt, i), selectPos(vs2Head.asUInt, i))
    } else {
      downOverflowSign(i) := Mux(isReNeg, selectPos(vs1Head.asUInt, i), selectPos(vs2Head.asUInt, i))
    }
  }
