...
ElementInput VGMIntegerALUF::select_vialuf_element(VecInput input, int idx, uint16_t mask, bool widenVd, bool widenVs2, bool isDstMask) {
  ...
  bool isSwap = ((input.fuOpType == VSCADDRQ_VV) || input.fuOpType == VSCADDRTQ_VV);
  ...
  switch (sew) {
    ...
    case 1:
      ...
      element.src2 = onlyWidenVs2 ? (uint64_t)input32->src2[idx] : (onlyWidenVd || isVf2) ? (uint64_t)input8->src2[idx] : isSwap ? (idx % 2 == 0) ? (uint64_t)input16->src2[idx+1] : (uint64_t)input16->src2[idx-1] : (uint64_t)input16->src2[idx];
...
VecOutput VGMIntegerALUF::get_expected_output(VecInput input) {
  ...
  bool isMisc = ((fuOpType & 0x20) >> 5) && !(fuOpType == VSCADDRQ_VV);
  bool isSat = (fuOpType == VSADDU_VV) || (fuOpType == VSADD_VV) || (fuOpType == VSSUBU_VV) || (fuOpType == VSSUB_VV) ||
               (fuOpType == VSCADDCONJ_VV) || (fuOpType == VSCADDRQ_VV) || (fuOpType == VSCADDRTQ_VV);
  ...
  for (int i = 0; i < number; i++) {
    ...
    switch (sew) {
      ...
      case 1: element.isRe = (i % 2 == 0) ? true : false; output_part[i] = calculation_e16(element); break;
...
ElementOutput VGMIntegerALUF::calculation_e16(ElementInput input) {
  ...
  case VSCADDCONJ_VV: {
    if (input.isRe) {
      ...
      output.result = (uint16_t)sat_sub<int16_t, uint16_t>(input.src1, input.src2, sat);
    }
    output.vxsat = sat;
    break; 
  }
